...
G4bool myStackingAction::InsideRoI(const G4Track * aTrack,G4double ang)
{
  if(!fMuonHits)
  { fMuonHits = (myMuonHitsCollection*)GetCollection("muonCollection"); }
  if(!fMuonHits)
...
G4VHitsCollection* myStackingAction::GetCollection(G4String colName)
{
...
////      G4cout << "++++++++ event aborted" << G4endl;
  ...
  //              in the tracker layers.
    nhits = fMuonHits->entries();
    if(!fTrkHits)
    { fTrkHits = (myPhotocathodeOPhotonHitsCollection*)GetCollection("photocathodeCollection"); }
    if(!fTrkHits)
    { G4cerr << "photocathodeCollection NOT FOUND" << G4endl;
      return; }
