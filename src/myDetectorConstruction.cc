...
void myDetectorConstruction::AddPhotocathodeSurfaceProperties(G4OpticalSurface *photocathode_opsurf) {
    ...
    G4double scaleFactor = 28.9415/12.0478/1.3246;
        for(int i = 0; i < nEntries2; i++)
            photocathode_EFF[i] *= 0.01/scaleFactor;
...
void myDetectorConstruction::AddRefCoatSurfaceProperties(G4OpticalSurface *refCoat_opsurf) {
    ...
    for(int i = nEntries; i--;) {
        coating_EFF[i] = 1.;
        coating_REF[i] = fCoatReflectivity;
    }
...
G4VPhysicalVolume* myDetectorConstruction::Construct()
{
    ...
    G4double world_sizeXY = 2.*(fRadius + fThicknessCost);
    G4double world_sizeZ  = 13.5*radiationlength;
    ...
    G4double globalZPosshift = 6*radiationlength;
...
// Radiator
    ...
    G4ThreeVector posRadiator = G4ThreeVector(0.*cm, 0.*cm, 0.*cm + globalZPosshift);
    G4double rminRadiator =  0.*cm;
    G4double rmaxRadiator =  fRadius;
...
//Reflective/Absortive Coat

    if (fCoat) {
        // G4Material* matRefCoat = nist->FindOrBuildMaterial("G4_Au");
        G4Material* matRefCoat = nist->FindOrBuildMaterial("G4_Galactic");
        G4double kdistanceCoat = fThicknessCost;

        G4Tubs* solidRefCoat =
          new G4Tubs("RefCoat", rmaxRadiator, rmaxRadiator + kdistanceCoat, 0.5*hzRadiator, phiminRadiator, phimaxRadiator);

        G4LogicalVolume* LVRefCoat = new G4LogicalVolume(solidRefCoat, matRefCoat, "RefCoat");
        new G4PVPlacement(0, posRadiator, LVRefCoat, "RefCoat", logicWorld, false, 0, checkOverlaps);

        //Optical surface
        G4OpticalSurface* RefCoat_opsurf = new G4OpticalSurface("refCoat_opsurf", glisur,polished, dielectric_metal);
        AddRefCoatSurfaceProperties(RefCoat_opsurf);
        new G4LogicalSkinSurface("refCoat_surf", LVRefCoat, RefCoat_opsurf);

        G4VisAttributes * VisAttRefCoat = new G4VisAttributes(G4Colour(1. ,0. ,1.));
        VisAttRefCoat->SetForceWireframe(true);
        LVRefCoat->SetVisAttributes(VisAttRefCoat);
    }

//Radiator Surface
    G4ThreeVector posRadiatorSurface = posRadiator;
    G4double thickofRadSurf = 1.*nm;
    posRadiatorSurface.setZ(posRadiatorSurface.z() - 0.5*hzRadiator - 0.5*thickofRadSurf);

    G4Tubs* solidRadiatorSurface = new G4Tubs("RadiatorSurface", 0.*cm, fRadius, 0.5*thickofRadSurf, phiminRadiator, phimaxRadiator);
    G4LogicalVolume* LVRadiatorSurface = new G4LogicalVolume(solidRadiatorSurface, world_mat, "RadiatorSurface");
    new G4PVPlacement(0, posRadiatorSurface, LVRadiatorSurface, "RadiatorSurface", logicWorld, false, 0, checkOverlaps);

    G4VisAttributes * VisAttRadiatorSurface = new G4VisAttributes(G4Colour(1. ,0. ,1.));
    // VisAttCalorimeterSurface->SetForceWireframe(true);
    LVRadiatorSurface->SetVisAttributes(VisAttRadiatorSurface);



    //----------------Photocathode/Cr Layer---------------------

    G4double rminPhotocathode =  0.*cm;
    G4double rmaxPhotocathode =  fRadius;
    G4double hzPhotocathode = fThick1effective;
    ...
    G4Material* matPhotocathode = nist->FindOrBuildMaterial("G4_Cr");
...
//CsI layer (PhotocathodeLayer2)
    G4ThreeVector posPhotocathodeLayer2 = posPhotocathode;
    G4double thickofPhotL2 = fThick2;
    posPhotocathodeLayer2.setZ(posPhotocathodeLayer2.z() + 0.5*hzPhotocathode + 0.5*thickofPhotL2);

    G4Material* matPhotocathodeLayer2 = nist->FindOrBuildMaterial("G4_CESIUM_IODIDE");
    G4Tubs* solidPhotocathodeLayer2 = new G4Tubs("PhotocathodeLayer2", 0.*cm, fRadius, 0.5*thickofPhotL2, phiminPhotocathode, phimaxPhotocathode);
    G4LogicalVolume* LVPhotocathodeLayer2 = new G4LogicalVolume(solidPhotocathodeLayer2, matPhotocathodeLayer2, "PhotocathodeLayer2");
    new G4PVPlacement(0, posPhotocathodeLayer2, LVPhotocathodeLayer2, "PhotocathodeLayer2", logicWorld, false, 0, checkOverlaps);

    G4VisAttributes * VisAttPhotocathodeLayer2 = new G4VisAttributes(G4Colour(1. ,0. ,1.));
    // VisAttCalorimeterSurface->SetForceWireframe(true);
    LVPhotocathodeLayer2->SetVisAttributes(VisAttPhotocathodeLayer2);

// photocathodeSurface

    G4ThreeVector posPhotocathodeSurface = posPhotocathodeLayer2;
    G4double thickofPhotSurf = 1.*nm;
    posPhotocathodeSurface.setZ(posPhotocathodeSurface.z() + 0.5*thickofPhotL2 + 0.5*thickofPhotSurf);

    G4Tubs* solidPhotocathodeSurface = new G4Tubs("PhotocathodeSurface", 0.*cm, 2*fRadius, 0.5*thickofPhotSurf, phiminPhotocathode, phimaxPhotocathode);
    G4LogicalVolume* LVPhotocathodeSurface = new G4LogicalVolume(solidPhotocathodeSurface, world_mat, "PhotocathodeSurface");
    new G4PVPlacement(0, posPhotocathodeSurface, LVPhotocathodeSurface, "PhotocathodeSurface", logicWorld, false, 0, checkOverlaps);

    G4VisAttributes * VisAttPhotocathodeSurface = new G4VisAttributes(G4Colour(1. ,0. ,1.));
    // VisAttCalorimeterSurface->SetForceWireframe(true);
    LVPhotocathodeSurface->SetVisAttributes(VisAttPhotocathodeSurface);


//calorimeter----------------------------------------------------

    G4double thickofCal = fLenMax*radiationlength;
    // G4double thickofCal = 0.000073*radiationlength;
    G4Material* matCalorimeter = nist->FindOrBuildMaterial("G4_W");
    G4ThreeVector posCalorimeter = G4ThreeVector(0.*cm, 0.*cm, 2*globalZPosshift - posRadiator[2] -0.5*hzRadiator - 0.5*thickofCal - 0.1*radiationlength);// - 6*radiationlength);
    // G4ThreeVector posCalorimeter = G4ThreeVector(0.*cm, 0.*cm, -3*radiationlength);// - 6*radiationlength);

    G4Tubs* solidCalorimeter = new G4Tubs("Calorimeter", 0.*cm, fRadius, 0.5*thickofCal, phiminPhotocathode, phimaxPhotocathode);
    ...
    G4VisAttributes * VisAttCalorimeter = new G4VisAttributes(G4Colour(0. ,1. ,1.));
    VisAttCalorimeter->SetForceWireframe(true);
    LVCalorimeter->SetVisAttributes(VisAttCalorimeter);

//calorimeter surface

    G4ThreeVector posCalorimeterSurface = posCalorimeter;
    G4double thickofCalSurf = 1.*nm;
    posCalorimeterSurface.setZ(posCalorimeterSurface.z() + 0.5*thickofCal + 0.5*thickofCalSurf);

    G4Tubs* solidCalorimeterSurface = new G4Tubs("CalorimeterSurface", 0.*cm, fRadius, 0.5*thickofCalSurf, phiminPhotocathode, phimaxPhotocathode);
    G4LogicalVolume* LVCalorimeterSurface = new G4LogicalVolume(solidCalorimeterSurface, world_mat, "CalorimeterSurface");
    new G4PVPlacement(0, posCalorimeterSurface, LVCalorimeterSurface, "CalorimeterSurface", logicWorld, false, 0, checkOverlaps);

    G4VisAttributes * VisAttCalorimeterSurface = new G4VisAttributes(G4Colour(0. ,1. ,1.));
    // VisAttCalorimeterSurface->SetForceWireframe(true);
    LVCalorimeterSurface->SetVisAttributes(VisAttCalorimeterSurface);
...
void myDetectorConstruction::ConstructSDandField()
{
    ...
    G4String photocathodeSurfaceSDname = "/mydet/photocathodeSurface";
    myPhotocathodeSurfaceSD * photocathodeSurfaceSD = new myPhotocathodeSurfaceSD(photocathodeSurfaceSDname);
    G4SDManager::GetSDMpointer()->AddNewDetector(photocathodeSurfaceSD);
    SetSensitiveDetector("PhotocathodeSurface",photocathodeSurfaceSD);

    G4String radiatorSurfaceSDname = "/mydet/radiatorSurface";
    myRadiatorSurfaceSD * radiatorSurfaceSD = new myRadiatorSurfaceSD(radiatorSurfaceSDname);
    G4SDManager::GetSDMpointer()->AddNewDetector(radiatorSurfaceSD);
    SetSensitiveDetector("RadiatorSurface",radiatorSurfaceSD);

    G4String calorimeterSurfaceSDname = "/mydet/calorimeterSurface";
    myCalorimeterSurfaceSD * calorimeterSurfaceSD = new myCalorimeterSurfaceSD(calorimeterSurfaceSDname);
    G4SDManager::GetSDMpointer()->AddNewDetector(calorimeterSurfaceSD);
    SetSensitiveDetector("CalorimeterSurface",calorimeterSurfaceSD);

    //set scorer in radiator
    G4String radiatorPSname = "/myscor/radiator";
    G4MultiFunctionalDetector* radiatorPS = new G4MultiFunctionalDetector(radiatorPSname);
    G4PSEnergyDeposit * radTotEnDep = new G4PSEnergyDeposit("TotalEnergyDeposit");
    G4PSDoseDeposit * radTotDoseDep = new G4PSDoseDeposit("TotalDoseDeposit");
    radiatorPS->RegisterPrimitive(radTotEnDep);
    radiatorPS->RegisterPrimitive(radTotDoseDep);
    G4SDManager::GetSDMpointer()->AddNewDetector(radiatorPS);
    SetSensitiveDetector("Radiator", radiatorPS );

    //set scorer in calorimeter
    G4String calorimeterPSname = "/myscor/calorimeter";
    G4MultiFunctionalDetector* calorimeterPS = new G4MultiFunctionalDetector(calorimeterPSname);
    G4PSEnergyDeposit * calTotEnDep = new G4PSEnergyDeposit("TotalEnergyDeposit");
    calorimeterPS->RegisterPrimitive(calTotEnDep);
    G4SDManager::GetSDMpointer()->AddNewDetector(calorimeterPS);
    SetSensitiveDetector("Calorimeter", calorimeterPS );
